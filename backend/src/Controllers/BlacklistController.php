...
class BlacklistController {
    ...
    public function create(Request $req) {
        ...
        foreach ($scopes as $scope) {
            ...
            $created[] = Database::fetchOne('SELECT * FROM anprc_blacklist_plates WHERE id = ?', [$id]);
        }
        if (!$created) {
            InspectionService::logOperation([
                'actor_username' => $actor, 'action' => 'blacklist.create',
                'request_payload' => $body, 'status' => 'failed',
                'error_message' => "Blacklist plate '$plate' already exists in the selected scope(s)",
            ]);
            Response::error("Blacklist plate '$plate' already exists in the selected scope(s)", 409);
            return null;
        }
        InspectionService::logOperation([
