...
class ChannelController {
    ...
    // paired_channel_id intentionally absent: lanes are not paired (enter from
    // any lane, exit from any lane); the DB column remains but is unused.
    private const EXTRA_FIELDS = ['uvis_timeout_sec', 'failure_audio_index', 'kind', 'location',
        // Per-lane road-blocker relay config (multi-lane).
        'blocker_relay_enabled', 'blocker_relay_topic', 'blocker_relay_pub_topic',
        'blocker_relay_res', 'blocker_relay_value', 'blocker_relay_open_ch',
        'blocker_relay_close_ch', 'blocker_relay_stop_ch', 'blocker_auto_open',
        // Per-lane behaviour switches (flow stages on/off). ANPR has no switch —
        // recognition + barrier is the lane's entry mechanism, never disabled.
        'behavior_uvis_s300', 'behavior_vip', 'behavior_blacklist',
        'behavior_whitelist_only',
        // S300 inspection mode (none/skip/timed/full) + the 'timed' countdown.
        's300_inspection_mode', 's300_timed_seconds',
        // Security mode: x-ray routing policy (red/orange/green) + orange's %.
        'security_mode', 'security_random_pct'];

    /**
     * Keep behavior_uvis_s300 in sync with the S300 inspection mode so existing
     * flow checks (cron hasS300, come() auto-pass, autoLeave) stay correct:
     * mode 'none' → 0 (no S300 cycle), any other mode → 1.
     */
    private static function syncUvisToggle(array &$data): void {
        if (array_key_exists('s300_inspection_mode', $data)) {
            $data['behavior_uvis_s300'] = ($data['s300_inspection_mode'] === 'none') ? 0 : 1;
        }
    }
