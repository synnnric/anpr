...
class DashboardController {
    ...
    private function probeRoadBlocker(array $channel): ?array {
        $perLane = (int)($channel['blocker_relay_enabled'] ?? 0) === 1;
        if ($perLane) {
            $enabled = true;
        } else {
        ...
        // Real liveness: online only if a relay heartbeat arrived within ~30s
        // (the CORX publishes a status every ~10s). age_sec is DB-consistent.
        $seen = Database::fetchOne(
            "SELECT blocker_last_seen, EXTRACT(EPOCH FROM (NOW() - blocker_last_seen))::int AS age_sec
             FROM anprc_channels WHERE id = ?", [$channel['id']]
        );
        $age = ($seen && $seen['age_sec'] !== null) ? (int)$seen['age_sec'] : null;
        $online = $enabled && $age !== null && $age <= 30;
        return [
            'enabled'   => $enabled,
            'online'    => $online,
            'last_seen' => $seen['blocker_last_seen'] ?? null,
            'age_sec'   => $age,
            'mode'      => 'corx_relay_mqtt',
            'position'  => $channel['blocker_position'] ?? 'unknown',  // down | up | unknown (ACK-confirmed)
        ];
    }
    ...
    private function todayStats(string $startUtc, string $endUtc): array {
        ...
        $visits = Database::fetchOne(
            "SELECT
                ...
                -- Overstay: still inside after 24h (status only — affects nothing).
                -- entry_at is naive-UTC (gmdate), so compare against UTC now.
                COUNT(*) FILTER (WHERE status = 'active'
                    AND entry_at < (NOW() AT TIME ZONE 'UTC') - INTERVAL '24 hours')::int AS overstay
