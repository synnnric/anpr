...
class InspectionController {
    ...
    public function show(Request $req) {
        ...
        $row['video_streams'] = Database::fetchAll(
            'SELECT id, camera_code, stream_url, stream_kind, received_at FROM anprc_inspection_video_streams WHERE inspection_id = ? ORDER BY id ASC',
            [$id]
        );

        $xray = Database::fetchAll(
            'SELECT * FROM anprc_inspection_xray WHERE inspection_id = ? ORDER BY id ASC',
            [$id]
        );
        foreach ($xray as &$x) {
            $x['scanned_image_url'] = ImageStorage::publicUrl($x['scanned_image_path']);
            $x['plate_image_url'] = ImageStorage::publicUrl($x['plate_image_path']);
            if (is_string($x['alarm_info'])) $x['alarm_info'] = json_decode($x['alarm_info'], true);
        }
        unset($x);
        $row['xray'] = $xray;
    ...
    /** POST /api/inspections/{id}/route-xray — operator manually sends this
     *  vehicle to the X-RAY (videotron text + routing record). */
    public function routeXray(Request $req) {
        $id = (int)$req->param('id');
        $actor = AuthController::usernameFromRequest($req);
        if (!$actor) { Response::error('authentication required', 401); return null; }
        ...
        $insp = Database::fetchOne('SELECT * FROM anprc_inspections WHERE id = ?', [$id]);
        if (!$insp) { Response::notFound('Inspection not found'); return null; }
        if (!empty($insp['xray_route'])) {
            Response::error('already routed to x-ray: ' . $insp['xray_route'], 409);
            return null;
        }

        $channel = Database::fetchOne('SELECT * FROM anprc_channels WHERE channel_no = ?', [$insp['channel_no']]);
        DecisionExecutor::routeToXray($insp, $channel ?? [], (string)$insp['decision'], 'manual', $actor);
