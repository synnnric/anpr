...
class MqttLogController {
    ...
    public function outbound(Request $req) {
        ...
        // Plate lives inside JSONB. Shapes seen on white_list_operator:
        //   flat:    payload->>'plate'
        //   single:  payload->'dldb_rec'->>'plate'      (dldb_rec is an object)
        //   bulk:    payload->'dldb_rec'->[i]->>'plate' (dldb_rec is an array)
        if ($plate) {
            $where[] = "(
                payload->>'plate' ILIKE :p
                OR payload->'dldb_rec'->>'plate' ILIKE :p
                OR EXISTS (
                    SELECT 1 FROM jsonb_array_elements(
                        CASE WHEN jsonb_typeof(payload->'dldb_rec') = 'array'
                             THEN payload->'dldb_rec' ELSE '[]'::jsonb END
                    ) AS rec WHERE rec->>'plate' ILIKE :p
                )
            )";
            $params['p'] = "%$plate%";
        }
