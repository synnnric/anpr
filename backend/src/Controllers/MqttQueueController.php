...
class MqttQueueController {
    ...
    /**
     * POST /api/mqtt-queue/clear?scope=pending|all — trash the outbound queue for
     * a fresh test run. Default 'pending' deletes the un-sent commands that would
     * otherwise replay (pending + failed); 'all' wipes the whole queue history.
     */
    public function clear(Request $req) {
        $scope = (string)$req->input('scope', 'pending');
        $stmt = $scope === 'all'
            ? Database::query("DELETE FROM anprc_mqtt_outbound_queue")
            : Database::query("DELETE FROM anprc_mqtt_outbound_queue WHERE status IN ('pending','failed')");
        return ['code' => 200, 'message' => 'cleared', 'data' => ['scope' => $scope, 'deleted' => $stmt->rowCount()]];
    }
