...
- ❌ Don't store the PostgreSQL superuser password in `config.php` — use the
  dedicated `anpr` user.
...
- **HA**: PHP backend + PostgreSQL replicated; worker can run active/passive
  (two workers with different `MQTT_CLIENT_ID` — only one acts on each plate if
  you add a simple distributed lock, e.g. via a PostgreSQL row).
