...
| `POST {s300_base_url}/recapture/{ch}`        | operator | force an extra capture |
| `POST {s300_base_url}/leave/{ch}`            | DecisionExecutor | release vehicle, begin reset |
...
> The vendor's deployed server diverges from the protocol PDF in two ways
> (verified against the real test server): capture is `POST /recapture` (doc
> said `GET /capture`), leave is `POST` (doc said `GET`). Responses come
> wrapped as `{success, message, result, error}` — failures arrive as HTTP 200
> with `success:false` (e.g. `设备未连接` = device not connected), which
> `S300Client` detects and reports as a failed call.
