...
| `POST {s300_base_url}/recapture/{ch}`        | operator | paksa capture tambahan |
| `POST {s300_base_url}/leave/{ch}`            | DecisionExecutor | lepaskan kendaraan, mulai reset |
...
> Server vendor yang ter-deploy menyimpang dari PDF protokol dalam dua hal
> (terverifikasi terhadap server testing asli): capture = `POST /recapture`
> (dokumen bilang `GET /capture`), leave = `POST` (dokumen bilang `GET`).
> Respons dibungkus `{success, message, result, error}` — kegagalan datang
> sebagai HTTP 200 dengan `success:false` (mis. `设备未连接` = perangkat tidak
> terhubung), yang dideteksi `S300Client` dan dilaporkan sebagai panggilan gagal.
