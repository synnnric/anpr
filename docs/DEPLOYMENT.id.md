...
MQTT_USER="admin"     # auth broker WAJIB (allow_anonymous false) — lihat §4.3
MQTT_PASS="ganti-mqtt-pass"
...
# Buat credential (user `admin` adalah konvensi yang dipakai di seluruh panduan ini)
sudo mosquitto_passwd -c -b /etc/mosquitto/passwd "${MQTT_USER:-admin}" "${MQTT_PASS:-ganti-mqtt-pass}"
...
# Verifikasi
mosquitto_sub -h localhost -u "${MQTT_USER:-admin}" -P "${MQTT_PASS:-ganti-mqtt-pass}" -t test &
mosquitto_pub -h localhost -u "${MQTT_USER:-admin}" -P "${MQTT_PASS:-ganti-mqtt-pass}" -t test -m hello
```

> **Autentikasi wajib.** Broker berjalan dengan `allow_anonymous false`, jadi
> **setiap** klien harus menyertakan credential (user `admin` di atas):
> - **Worker** — `MQTT_USERNAME`/`MQTT_PASSWORD` di `/opt/anpr-worker/.env` (§4.4).
> - **Kamera** — set username/password di konfigurasi MQTT masing-masing kamera.
> - **Panel live dashboard** (MQTT-over-WebSocket di :8083) — masukkan credential
>   di halaman **Koneksi MQTT**.
> - **Debug** — tambahkan `-u admin -P <pass>` ke setiap `mosquitto_sub`/`mosquitto_pub`.
>
> File `password_file` harus **dapat dibaca oleh akun yang menjalankan broker** —
> pada instalasi systemd/Debian ini akun tersebut adalah user `mosquitto`
> (perintah `chown` di atas); di OS lain (mis. service Windows yang berjalan
> sebagai LocalSystem) beri akun itu akses baca, atau broker gagal start selama
...
#   MQTT_USERNAME=admin            # wajib — harus cocok dengan password file broker
...
4. **Settings** → aktifkan **Auto-start S300 Inspection** agar plat otomatis memicu /come.
   - *Opsional* **gerbang masuk** — jika kamera ANPR masuk menggerakkan palangnya
     sendiri lewat relay GPIO, set `entry_gate_open=1` agar platform memberi pulsa
     saat plat dikenali (di `/come`, via `gpio_out`). Atur `entry_gate_io` (indeks
     output 0–3, default `0`), `entry_gate_value` (`0`=off, `1`=on, `2`=pulse —
     default `2`) dan `entry_gate_pulse_ms` (default `1000`). Biarkan
     `entry_gate_open=0` (default) bila palang masuk hanya road blocker saja.
...
- ❌ Jangan edit manual baris `anprc_inspections` saat worker hidup (kontensi lock +
  race decision).
- ❌ Jangan simpan password superuser PostgreSQL di `config.php` — pakai user `anpr` khusus.
...
- **HA**: Backend PHP + PostgreSQL direplikasi; worker bisa jalan active/passive
  (dua worker dengan `MQTT_CLIENT_ID` berbeda — hanya satu yang beraksi per plat
  jika Anda tambah simple distributed lock, mis. via row PostgreSQL).
