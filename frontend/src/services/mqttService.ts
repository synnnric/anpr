...
class MqttService {
  ...
  /**
   * Try a connection with the given config WITHOUT touching the live client or
   * status listeners — used by the "Test Connection" button. Resolves on a
   * successful CONNECT (creds accepted), rejects with the broker error / a
   * timeout. Always tears its throwaway client down.
   */
  testConnect(config: MqttConfig, timeoutMs = 8000): Promise<void> {
    return new Promise((resolve, reject) => {
      const protocol = config.useSSL ? 'wss' : 'ws';
      const url = `${protocol}://${config.brokerUrl}:${config.port}/mqtt`;
      const probe = mqtt.connect(url, {
        clientId: (config.clientId || 'anpr_dashboard') + '_test_' + Date.now(),
        username: config.username || undefined,
        password: config.password || undefined,
        clean: true,
        reconnectPeriod: 0,   // one shot — do not retry
      ...
      const timer = setTimeout(() => finish(new Error('Connection timeout')), timeoutMs + 500);
      probe.on('connect', () => finish());
      probe.on('error', (err) => finish(err instanceof Error ? err : new Error(String(err))));
    });
  }
