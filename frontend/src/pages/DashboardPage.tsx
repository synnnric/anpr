      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {snap && <>
          ...
          <section>
            ...
            {(['entry', 'exit', 'xray'] as const).map(kind => {
              const chs = snap.channels.filter(c => c.kind === kind);
              if (chs.length === 0) return null;
              return (
                <div key={kind} className="mb-4 last:mb-0">
                  <h3 className={`text-xs font-semibold uppercase tracking-wide mb-2 ${
                    kind === 'entry' ? 'text-blue-300' : kind === 'exit' ? 'text-purple-300' : 'text-red-300'
                  }`}>
                    {t(kind === 'entry' ? 's300.ch.group.entry' : kind === 'exit' ? 's300.ch.group.exit' : 's300.ch.group.xray')}
                    <span className="text-text-secondary font-normal ml-1">({chs.length})</span>
                  </h3>
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                    {chs.map(ch => <ChannelCard key={ch.channel_no} ch={ch} />)}
                  </div>
                </div>
              );
            })}
...
function flattenDevices(channels: DashboardChannel[], labels: { anpr: string; s300: string; rb: string; unreachable: string; rb_no_heartbeat: string; rb_never: string }) {
  type Row = { key: string; channel: string; kind: string; type: string;
  ...
  for (const c of channels) {
    if (c.anpr_device_sn) {
      rows.push({
        ...
        channel: c.channel_no, kind: c.kind, type: labels.anpr, icon: Camera,
        label: `ANPR (${c.kind})`,
        online: c.anpr_status === 'ok',
        // Show WHICH camera this lane maps to (SN) next to its liveness.
        detail: `SN ${c.anpr_device_sn} · ${fmtIdleTime(c.anpr_last_heartbeat_at)}`,
      });
    }
    if (c.kind === 'entry' && c.s300) {
      rows.push({
        key: `${c.channel_no}-s300`,
        channel: c.channel_no, kind: c.kind, type: labels.s300, icon: ShieldCheck,
        label: labels.s300,
    ...
    if (c.kind === 'entry' && c.road_blocker) {
      rows.push({
        key: `${c.channel_no}-rb`,
        channel: c.channel_no, kind: c.kind, type: labels.rb, icon: Columns3,
        label: labels.rb,
        online: c.road_blocker.online,
        detail: !c.road_blocker.enabled ? labels.unreachable
          : c.road_blocker.online
            ? `MQTT relay · ${c.road_blocker.age_sec ?? 0}s ago`
            : (c.road_blocker.last_seen ? labels.rb_no_heartbeat : labels.rb_never),
      });
    }
  }
  return rows;
...
function DeviceList({ channels }: { channels: DashboardChannel[] }) {
  ...
  const devices = flattenDevices(channels, {
    ...
    rb_no_heartbeat: t('dashboard.rb.no_heartbeat'),
    rb_never: t('dashboard.rb.never'),
  });
  if (devices.length === 0) return null;
  const mqttOk = mqttStatus === 'connected';
  // One row per channel so each lane's devices read left-to-right, with the
  // channels grouped by kind (entry lanes first, then exit).
  const channelNos: string[] = [];
  const byChannel = new Map<string, typeof devices>();
  const channelKind = new Map<string, string>();
  for (const d of devices) {
    if (!byChannel.has(d.channel)) {
      byChannel.set(d.channel, []); channelNos.push(d.channel); channelKind.set(d.channel, d.kind);
    }
    byChannel.get(d.channel)!.push(d);
  }
  return (
    <div className="mt-3 space-y-1.5">
      {/* This CP's own MQTT (websocket) link — moved here from the removed Connection page */}
      <div className="flex flex-wrap items-center gap-2">
        <span className="w-16 shrink-0" />
        <div className={`flex items-center gap-2 border rounded-md px-2 py-1 text-[11px] ${
          mqttOk ? 'border-green-500/30 bg-green-500/5' : 'border-red-500/30 bg-red-500/5'
        }`}>
          <span className={`w-1.5 h-1.5 rounded-full ${mqttOk ? 'bg-green-400' : 'bg-red-400'}`} />
          <Radio className="w-3 h-3 text-text-secondary" />
          <span className="font-mono text-text-primary">MQTT</span>
          <span className="text-text-secondary">·</span>
          <span className="text-text-secondary">{mqttOk ? t('dashboard.mqtt.connected') : t('dashboard.mqtt.disconnected')}</span>
        </div>
      </div>
      {(['entry', 'exit', 'xray'] as const).map(kind => {
        const chs = channelNos.filter(ch => channelKind.get(ch) === kind);
        if (chs.length === 0) return null;
        return (
          <div key={kind} className="space-y-1.5">
            <div className={`pt-1 text-[10px] font-semibold uppercase tracking-wide ${
              kind === 'entry' ? 'text-blue-300' : kind === 'exit' ? 'text-purple-300' : 'text-red-300'
            }`}>
              {t(kind === 'entry' ? 's300.ch.group.entry' : kind === 'exit' ? 's300.ch.group.exit' : 's300.ch.group.xray')}
            </div>
        ...
        <div key={ch} className="flex flex-wrap items-center gap-2">
          <span className="w-16 shrink-0 text-[11px] font-mono font-semibold text-text-secondary text-right">{ch}</span>
          {byChannel.get(ch)!.map(d => {
            const Icon = d.icon;
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <Stat icon={Clock}         label={t('dashboard.today.overstay')} value={td.visits.overstay}      color="text-orange-400" />
      </div>
    </section>
  );
}
      ...
      <div className="flex items-center justify-between mb-3 pb-3 border-b border-border">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            {ch.location && (
              <span className="text-[10px] px-1.5 py-0.5 rounded bg-primary/10 text-primary-light border border-primary/30">{ch.location}</span>
            )}
