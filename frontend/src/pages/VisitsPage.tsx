...
import { Search, RefreshCw, Car, LogIn, LogOut, AlertTriangle, ShieldAlert, DoorOpen, Ban, Loader2 } from 'lucide-react';
import { listVisits, visitSummary, allowOrphanExit, denyOrphanExit } from '../services/s300Service';
...
export default function VisitsPage() {
  ...
  const decideOrphan = async (id: number, allow: boolean) => {
    setOrphanBusy(id);
    try {
      if (allow) await allowOrphanExit(id);
      else await denyOrphanExit(id);
      await reload();
    } catch { /* row state refreshes on the next poll */ }
                ...
                return (
                  <tr key={v.id} className="border-t border-border">
                    ...
                    <td className="px-4 py-2">
                      ...
                      {v.status === 'orphan_exit' && v.orphan_review === 'pending' && (
                        <div className="flex items-center gap-1 mt-1.5">
                          <button onClick={() => decideOrphan(v.id, true)} disabled={orphanBusy !== null}
                            title={t('orphan.alert.allow')}
                            className="flex items-center gap-1 px-1.5 py-0.5 text-[10px] bg-green-600 hover:bg-green-500 text-white rounded disabled:opacity-50">
                            {orphanBusy === v.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <DoorOpen className="w-3 h-3" />}
                            {t('orphan.row.allow')}
                          </button>
                          <button onClick={() => decideOrphan(v.id, false)} disabled={orphanBusy !== null}
                            title={t('orphan.alert.deny')}
                            className="flex items-center gap-1 px-1.5 py-0.5 text-[10px] border border-border text-text-secondary hover:text-danger rounded disabled:opacity-50">
                            <Ban className="w-3 h-3" /> {t('orphan.row.deny')}
                          </button>
                        </div>
                      )}
                      {v.status === 'orphan_exit' && (v.orphan_review === 'allowed' || v.orphan_review === 'denied') && (
                        <div className={`text-[10px] mt-1 ${v.orphan_review === 'allowed' ? 'text-green-400' : 'text-red-400'}`}>
                          {t(v.orphan_review === 'allowed' ? 'orphan.row.allowed_by' : 'orphan.row.denied_by', { by: v.orphan_reviewed_by ?? '?' })}
                        </div>
                      )}
                    </td>
